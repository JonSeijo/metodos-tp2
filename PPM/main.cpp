...
void test_image(std::string s_filename, string input_path){
  uchar* data = NULL;
  ...
  ofstream outresfile;
  outresfile.open(input_path, std::ios_base::app);

  for (int h = 0; h < height; ++h){
    for (int w = 0; w < width; ++w){
      outresfile << get_pixel_average(data, h, w, height, width)
      << (w + 1 == width && h + 1 == height ? "" : ","); // Ejemplo de lectura de un pixel
    }
    // cout << endl;
  }
  outresfile << "\n";
...
void test_load(std::string s_filename){
...
void test_save(){
  ...
  char comments[100];
  sprintf(comments, "%s", "Este string se inserta como comentario en el .ppm");
...
int main() {
  ...
  std::string filename = "jonnov2.ppm";
  ...
  ofstream outresfile;
  outresfile.open("input_custom.csv", fstream::out);
  outresfile << HEADER << "\n";
  outresfile.close();

  for (string filename : filenames) {
    test_load(filename);
    test_image(filename, "input_custom.csv");
  }
